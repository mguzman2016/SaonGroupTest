...
object ETLProcess extends SparkBuilder{
  ...
  def main(args: Array[String]): Unit = {
    ...
    database.insertDfIntoDB(if(!debugging){ datesDf.select(datesDfColumns: _*) }else{ datesDf.select(datesDfColumns: _*).limit(100) },schema,datesAdvertTableName,jdbcString)
    if(suppressMessages){
